...
function extract_subtitle {
	...
	local SUBTITLE_INDEX=$(ffprobe -v error -select_streams s -show_entries stream=index:stream_tags=language -of csv=p=0 "${VIDEO_FILE}" | awk -F, -v lang="${LANGUAGE_CODE}" '$2==lang {print $1-2}')
